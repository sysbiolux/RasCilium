...
optLocalParams=nan(numel(indExpToOpt),size(paramdatalocal,1),repetition);
...
for i=1:repetition
    ...
    estim.optimization.options.maxfunevals = 10000*fitLonger;
    ...
    estim.optimization.options.maxfunevals = 3000*fitLonger;
...
plotstates=[1:numel(IQMstates(model))]; % plot all states
...
for j=1:length(optimalcost)
    ...
    if (optimalcost(j)<threshold) % if optimalcost pass threshold
        ...
        model2 = IQMparameters(model2,output.parameterslocal,optLocalParams(counter2,:,j));
...
for counter=1:numel(indExpToOpt)
    ...
    disp(['Experiment: ' num2str(indExpToOpt(counter))])
    disp('mean:')
    mean(temp)
    disp('std:')
    std(temp)
...
% figure
figure('color','white')
for counter=1:size(res,2)
    subplot(2,3,counter)
    boxplot(squeeze(res(:,counter,:)),'Labels',{'Ctrl','Kras','Nras','Hras'})
    xtickangle(45)
    temp=IQMstates(model);
    title(temp(counter))
    temp=ylim;
    ylim([0 temp(2)])
end
...
for counter=1:numel(indExpToOpt)
    figure
    temp=optLocalParams(:,:,optimalcost<=threshold);
    temp=squeeze(temp(counter,:,:))';
    boxplot(temp)
    ylabel({'local parameter value'},'fontweight','bold','fontsize',12)
    set(gca,'xtick',1:size(optParams,2),'xticklabel',paramdatalocal(:,1),'fontweight','bold','fontsize',12);
    xtickangle(45)
    title(['Experiment: ' num2str(indExpToOpt(counter))])
end
...
figure
res=[];
for counter=2:numel(indExpToOpt)
    temp=optLocalParams(:,:,optimalcost<=threshold);
    temp=squeeze(temp(counter,:,:))';
    res=[res, temp'];
end
...
xtickangle(45)
% title(['Experiment: ' num2str(indExpToOpt(counter))])
title(paramdatalocal(:,1))
hold on
disp('fRasCil over exp4-6')
disp('mean:')
mean(res)
disp('std:')
std(res)
