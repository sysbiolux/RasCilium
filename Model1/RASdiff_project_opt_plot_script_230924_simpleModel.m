...
boundFactor= 3 %1.2
paramdata = {
   'sP_PT'     1.16/boundFactor      1.16*boundFactor
   'xPT'     0       1
   'mT'     1.36/boundFactor      1.36*boundFactor
   'mTD'     0       1*boundFactor
   'd'     0       1*boundFactor
   };
...
for i=1:repetition
    ...
    estim.optimization.options.maxfunevals = 10000;
    ...
    estim.optimization.options.maxfunevals = 3000;
...
plotvariables=[2:numel(IQMvariables(model))]; % plot all variables except serumSwitchTime
...
for j=1:length(optimalcost)
    ...
    if (optimalcost(j)<threshold) % if optimalcost pass threshold
        ...
        model = IQMparameters(model,output.parameters,optParams(j,:));
        simulation = IQMsimulate(model,5);
        
        % start plotting each state for each parameter set
        for k=1:length(plotstates);
            plotstate=plotstates(k);
            subplot(3,3,k), hold on, plot(simulation.time,simulation.statevalues(:,plotstate),'LineWidth',2),
            legend(simulation.states(plotstate)), hold off
            hold on
        end
        for k2=1:length(plotvariables);
            plotstate=plotvariables(k2);
            subplot(3,3,k+k2), hold on, plot(simulation.time,simulation.variablevalues(:,plotstate),'LineWidth',2),
...
for counter=1:numel(componentNames)
    if isempty(find(ismember(simulation.variables,componentNames(counter))))
       temp=find(ismember(simulation.states,componentNames(counter)));
        ...
        temp=find(ismember(simulation.variables,componentNames(counter)))+k-1;
    end
    subplot(3,3,temp), hold on, errorbar(time,values(:,counter),values(:,counter)-minvalues(:,counter),maxvalues(:,counter)-values(:,counter),'r.','MarkerSize',20), hold off
hold on 
